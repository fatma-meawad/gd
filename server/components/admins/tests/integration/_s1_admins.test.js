...
describe('Test suite for /s1/admins', () => {
 beforeAll(async () => {
  await initializeApp(); // Reuse the initialized app
    ...
    describe('Test suite for get /s1/admins', () => {


        test('Test case: /s1/admins with Request Example: ValidExample', async () => {
  ...
  "page": ""
        ...
        test('Test case: /s1/admins with Request Example: InvalidExample', async () => {
          const response = await request(app)
              .get(baseUrl +'/s1/admins')
               .set('Accept', 'application/json')
              .query({
